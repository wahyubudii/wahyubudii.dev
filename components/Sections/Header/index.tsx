...
export default function Header() {
    ...
    const socialMedia = [
        { name: "Github", url: "https://github.com/wahyubudii", ions: "AiFillGithub"},
                            ...
                            { socialMedia.map((val, index) => {
                                return (
                                    <Link key={index} href={val.url} target={'_blank'}>
                                        { val.name === "Github" ? <AiFillGithub className='text-black h-6 w-6' /> : <AiFillLinkedin className='text-black h-6 w-6' />}
                                    </Link>
                                )
                            })}
                        </div>
